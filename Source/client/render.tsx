...
export class CellOutput extends React.Component<ICellOutputProps> {
	...
	private renderImage(
		...
		if (this.props.ctx.postMessage) {
			...
			} as IsJupyterExtensionInstalled);
		}
		if (this.props.ctx.onDidReceiveMessage) {
			const disposable = this.props.ctx.onDidReceiveMessage(
				(response: Partial<IsJupyterExtensionInstalled>) => {
					if (
						response?.type === "isJupyterExtensionInstalled" &&
						response.response
					) {
						(globalThis as any).__isJupyterInstalled =
							response.response;
					}
				}
			);
		...
		const divStyle: Record<string, string | number> = {
			overflow: "scroll",
			position: "relative",
		}; // `overflow:scroll` is the default style used by Jupyter lab.
		const imgSrc =
		...
		if (
			customMetadata &&
			typeof customMetadata.needs_background === "string"
		) {
	...
	private renderOutput(
		data: nbformat.MultilineString | PartialJSONObject,
		mimeType?: string
	) {
