...
function fixJupyterLabRenderers() {
	...
	fs.writeFileSync(
		filePath,
		fileContents.replace(textToReplace, textToReplaceWith)
	);
}
...
function fixPlotlyRenderer() {
	const filePath = path.join(
		__dirname,
		"..",
		"node_modules/@nteract/transform-plotly/lib/index.js"
	);
	...
	const fileContents = fs.readFileSync(filePath, "utf8").toString();
	if (
		fileContents.indexOf(textsToReplace[0]) === -1 &&
		fileContents.indexOf(textsToReplaceWith[0]) === -1
	) {
		throw new Error("Unable to find plotly fixup");
	}
	let replacedFileContents = fileContents;
	textsToReplace.forEach((s, i) => {
		replacedFileContents = replacedFileContents.replace(
			s,
			textsToReplaceWith[i]
		);
	});
